...
class DetectZoneWired(UIBase):

    # used in tearDown to update test status on zephyr
    tune_app = TuneElectron()
    device_name = zone_wired_api.name
    conn_type = ConnectionType.dongle

    def test_XXX_VC_103770_detect_zone_wired(self):
        try:
            disconnect_all()
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(DetectZoneWired)
