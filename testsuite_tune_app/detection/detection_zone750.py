...
from testsuite_firmware_api_tests.api_tests.api_parameters import zone_750_api
...
class DetectZone750(UIBase):
    # used in tearDown to update test status on zephyr
    tune_app = TuneElectron()
    device_name = zone_750_api.name
    conn_type = ConnectionType.dongle

    def test_XXX_VC_103769_detect_zone_750(self):
        try:
            disconnect_all()
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(DetectZone750)
