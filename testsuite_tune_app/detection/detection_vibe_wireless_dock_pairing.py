...
from testsuite_firmware_api_tests.api_tests.api_parameters import logi_dock_api, zone_vibe_wireless_api

log = logging.getLogger(__name__)
directory = os.path.dirname(os.path.dirname(__file__))


class DetectZoneVibeWirelessDockPairing(UIBase):

    dock_name = logi_dock_api.name
    device_name = zone_vibe_wireless_api.name
    receiver_name = zone_vibe_wireless_api.receiver_name
    ...
    def test_XXX_VC_YYYYY_detect_zone_vibe_wireless_dock_pairing(self):
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(DetectZoneVibeWirelessDockPairing)
