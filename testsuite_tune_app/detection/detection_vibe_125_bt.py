...
from common.framework_params import ENDURO_RELAY_BOARD_SERIAL_NUMBER, ENDURO_POWER_ON_BUTTON, ENDURO_POWER_OFF_BUTTON, ENDURO_POWER_PAIR_BUTTON
...
class DetectZoneVibe125Bt(UIBase):
    ...
    def setUp(self):
        ...
        """
        try:
            super(DetectZoneVibe125Bt, self).setUp()
            ...
            # Communicate with the relay connected with Zone Vibe 125
            relay_com_port = self.ryCtrl.get_relay_com_port(ENDURO_RELAY_BOARD_SERIAL_NUMBER)
            self.ryCtrl.connect_relay(relay_com_port)

        except Exception as e:
            log.error('Unable to setUp test_XXX_VC_YYYYY_detect_vibe125_bt')
            raise e
    ...
    def tearDown(self):
        ...
        self.ryCtrl.press_btn(ENDURO_POWER_OFF_BUTTON, 3)
        self.ryCtrl.press_btn(ENDURO_POWER_ON_BUTTON, 1)
        ...
        self.btCtrl.bluetooth_unpair(zone_vibe_125_api.headset_bt_address, zone_vibe_125_api.name)

        super(DetectZoneVibe125Bt, self).tearDown()

    def test_XXX_VC_123389_detect_zone_vibe_125_bt(self):
        """Verification of detection of Zone Vibe 125 via bluetooth
        This test pairs and PC with Zone Vibe 125 via bluetooth and check the info on Tune
        It will fail if Tune does not show Zone Vibe 125 on the page
        """
        device_name = zone_vibe_125_api.name
        ...
        try:
            ...
            for i in range(1, 3):
                ...
                self.ryCtrl.press_btn(ENDURO_POWER_ON_BUTTON, 1)
                self.ryCtrl.press_btn(ENDURO_POWER_PAIR_BUTTON, 3)
    ...
    def test_XXX_VC_103780_power_on_off_zone_vibe_125_bt(self):
        """Verification of detection of Zone Vibe 125 via bluetooth after power on/off"""
        ...
        try:
            ...
            self.ryCtrl.press_btn(power_on_btn, 1)
            time.sleep(3)
            self.ryCtrl.press_btn(pair_btn, 3)
            time.sleep(3)
            self.btCtrl.bluetooth_pair(device_mac, device_name)
            if get_custom_platform() == "macos":
                self.btCtrl.bluetooth_connect(device_name)
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(DetectZoneVibe125Bt)
