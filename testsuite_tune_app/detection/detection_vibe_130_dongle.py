...
class DetectZoneVibe130Dongle(UIBase):
    # used in tearDown to update test status on zephyr
    tune_app = TuneElectron()
    device_name = zone_vibe_130_api.name
    conn_type = ConnectionType.dongle

    def test_XXX_VC_103775_detect_zone_vibe_130_dongle(self):
        """Verification of detecting Zone Vibe 130 through dongle connection
        This test powers off dongle which is plugged on USB hub and then powers on to check Zone Vibe 130 connection
        It will fail if the battery sign is not showed up in Zone Vibe 130 page on Tune
        """
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(DetectZoneVibe130Dongle)
