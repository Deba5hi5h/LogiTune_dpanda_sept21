...
class UpdateZoneVibe125(FwuStressBase):
    """
    Suite class for Zone Vibe 125 connected over BT FWU tests via EasterEgg and OTA.
    """
    ...
    def setUpClass(cls) -> None:
        super(UpdateZoneVibe125, cls).setUpClass()
        cls.device_name = zone_vibe_125.device_name
        global_variables.test_category = f'FW Update Stress {cls.device_name}'
        zone_vibe_125.is_initialized(skip_dongle=True)
        cls.baseline_version_headset = zone_vibe_125.baseline_device_version
        cls.target_version_headset = zone_vibe_125.target_device_version
        cls.file_path_headset = os.path.join(
            DIR_PATH, f"ZoneVibe125_Headset_{cls.baseline_version_headset}.img"
        )
        cls.file_path_target = os.path.join(
            DIR_PATH, f"ZoneVibe125_Headset_{cls.target_version_headset}.img"
        )

        t_files = FirmwareDownloader()
        t_files.prepare_firmware_files_for_test(S3_FOLDER, cls.file_path_headset, cls.file_path_target)

    @parameterized.expand([(x,) for x in range(1, zone_vibe_125.repeats + 1)])
    def test_209_VC_58905_update_zone_vibe_125_directBT(self, retry):
        """
        Scenario:
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(UpdateZoneVibe125)
