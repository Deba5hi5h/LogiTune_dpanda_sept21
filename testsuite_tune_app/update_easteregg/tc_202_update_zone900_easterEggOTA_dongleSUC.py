...
if JENKINS_FWU_CONFIG:
    from testsuite_tune_app.update_easteregg.device_parameters_jenkins import zone_900
...
class UpdateZone900Dongle(FwuStressBase):
    """
    Suite class for Zone 900 connected over SU dongle FWU tests via EasterEgg and OTA.
    """
    ...
    def setUpClass(cls) -> None:
        super(UpdateZone900Dongle, cls).setUpClass()

        global_variables.test_category = f'FW Update Stress {cls.device_name}'
        zone_900.is_initialized()

        cls.file_path_headset = os.path.join(
            DIR_PATH, f"zone900_headset_{cls.baseline_device_version}.bin")

        cls.file_path_dongle = os.path.join(
            DIR_PATH, f"zone900_dongle_suc_{cls.baseline_dongle_version}.dfu")

        if JENKINS_FWU_CONFIG:
            connect_device(device_name=cls.device_name)

        t_files = FirmwareDownloader()
        t_files.prepare_firmware_files_for_test(S3_FOLDER, cls.file_path_headset, cls.file_path_dongle)

    @classmethod
    def tearDownClass(cls) -> None:
        if JENKINS_FWU_CONFIG:
            disconnect_device(device_name=cls.device_name)
        super(UpdateZone900Dongle, cls).tearDownClass()

    @parameterized.expand([(x, ) for x in range(1, REPEATS + 1)])
    def test_202_VC_58901_update_firmware_zone_900_headset_and_dongle(self, retry):
        """
        Scenario:
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(UpdateZone900Dongle)
