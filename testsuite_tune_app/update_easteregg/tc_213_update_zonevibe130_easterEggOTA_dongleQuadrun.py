...
if JENKINS_FWU_CONFIG:
    from testsuite_tune_app.update_easteregg.device_parameters_jenkins import zone_vibe_130
...
class UpdateZoneVibe130QuadrunDongle(FwuStressBase):
    """
    ...
    def setUpClass(cls) -> None:
        super(UpdateZoneVibe130QuadrunDongle, cls).setUpClass()

        global_variables.test_category = f'FW Update Stress {cls.device_name}'
        zone_vibe_130.is_initialized()

        cls.file_path_headset = os.path.join(DIR_PATH, f"ZoneVibe130_Headset_{cls.baseline_device_version}.img")
    ...
    def tearDownClass(cls) -> None:
        ...
        super(UpdateZoneVibe130QuadrunDongle, cls).tearDownClass()

    @parameterized.expand([(x, ) for x in range(1, REPEATS + 1)])
    def test_213_VC_74094_update_firmware_zone_vibe_130_headset_and_dongle(self, retry):
        """
        Scenario:
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(
        UpdateZoneVibe130QuadrunDongle)
