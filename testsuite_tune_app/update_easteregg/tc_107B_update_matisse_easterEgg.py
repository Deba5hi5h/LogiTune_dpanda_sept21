...
class UpdateMatisseViaEasterEgg(FwuStressBase):
    """
    Suite class for Brio Pro 70X FWU tests via EasterEgg.
    """
    ...
    def setUpClass(cls) -> None:
        super(UpdateMatisseViaEasterEgg, cls).setUpClass()
        ...
        cls.file_path_target = os.path.join(
            DIR_PATH, f"{cls.build_type.value}{cls.target_device_version}.bin"
        )
    ...
    def tearDownClass(cls) -> None:
        ...
        super(UpdateMatisseViaEasterEgg, cls).tearDownClass()

    @parameterized.expand([(x,) for x in range(1, REPEATS + 1)])
    def test_107B_VC_100788_update_firmware_matisse(self, retry):
        """
        Scenario:
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(UpdateMatisseViaEasterEgg)
