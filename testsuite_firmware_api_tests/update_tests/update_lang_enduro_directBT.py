...
class UpdateLanguageEnduroBTConnection(UIBase):

    @classmethod
    def setUpClass(cls) -> None:
        super(UpdateLanguageEnduroBTConnection, cls).setUpClass()
        cls.tunesApp = TuneElectron()

    @parameterized.expand([(loop, ver) for loop in range(1, 2) for ver in LANGS])
    def test_XXX_VC_YYYYY_upgrade_language_enduro_firmware_dongle_connection(self, loop_no, ver):
    ...
    def __install_enduro_lang(self, device_mac, ver):
        ...
        if PLATFORM == 'windows':
            cmd = f"{firmware_directory}\\updater\\FWU_Sample.exe bt_spp_audio_data {device_mac} {ver[1]}"
        else:
            cmd = f"{firmware_directory}/updater/LGT_HID_E_FW_Update_CLT bt_spp_audio_data {device_mac} {ver[1]}"
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(UpdateLanguageEnduroBTConnection)
