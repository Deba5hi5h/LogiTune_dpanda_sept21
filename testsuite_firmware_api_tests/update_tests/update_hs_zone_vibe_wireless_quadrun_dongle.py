...
class InstallZoneVibeWirelessFwDirectBT(UIBase):

    @classmethod
    def setUpClass(cls):
        super(InstallZoneVibeWirelessFwDirectBT, cls).setUpClass()

        baseline_file = os.path.join(
            firmware_directory, f"ZoneVibeWireless_Headset_{DEVICE.baseline_device_version}.img"
        )
        target_file = os.path.join(
            firmware_directory, f"ZoneVibeWireless_Headset_{DEVICE.target_device_version}.img"
        )
    ...
    def tearDownClass(cls):
        super(InstallZoneVibeWirelessFwDirectBT, cls).tearDownClass()
    ...
    def test_XXX_VC_YYYYY_install_Enduro_FW(self, retry):
        ...
        try:
            ...
            Report.logInfo(f"Start Zone Vibe Wireless FW {fw_version} installation.")
            self.start_performance_test()
            self._install_enduro_fw(DEVICE_ADDRESS, fw_version)
            self.end_performance_test(f"Zone Vibe Wireless FW installation {fw_version} finished!")
    ...
    def _install_enduro_fw(self, device_address, fw_version):
        Report.logInfo(f"Installing Zone Vibe Wireless FW: {fw_version}")
        if PLATFORM == 'windows':
            cmd = f"{updater_directory}\\zone_vibe_updater_win\\FWU_Sample.exe hid " \
                  f"{device_address} {firmware_directory}\\" \
                  f"ZoneVibeWireless_Headset_{fw_version}.img"
        else:
            cmd = f"{updater_directory}/zone_vibe_updater_mac/LGT_HID_E_FW_Update_CLT hid " \
                  f"{device_address} {firmware_directory}/ZoneVibeWireless_Headset_{fw_version}.img"
        ...
        try:
            ...
            Report.logInfo(f"Zone Vibe Wireless Firmware installation to {fw_version} "
    ...
    def _verify_fw_version(device_address, fw_version):
        Report.logInfo(f"Checking Zone Vibe Wireless FW version: {fw_version}")
        if PLATFORM == 'windows':
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(InstallZoneVibeWirelessFwDirectBT)
