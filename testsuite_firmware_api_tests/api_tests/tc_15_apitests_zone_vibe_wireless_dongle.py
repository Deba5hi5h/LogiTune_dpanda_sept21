...
from testsuite_firmware_api_tests.api_tests.headsets_earbuds_firmware_api.device_config_zone_vibe_wireless import (
    FEATURES_ZONE_VIBE_WIRELESS, EQ_MODES, CONNECTED_DEVICES, CONNECTED,
    NOT_CONNECTED)


class ZoneVibeWirelessApiDongleTests(UIBase):
    """
    ...
    @classmethod
    def setUpClass(cls):
        super(ZoneVibeWirelessApiDongleTests, cls).setUpClass()
    ...
    def test_1510_VC_80458_set_and_get_device_name(self) -> None:
        try:
            NAME = f"{zone_vibe_wireless_api.name} {random.randint(0, 1000000)}"
    ...
    def test_1531_VC_80479_get_connected_device_name(self) -> None:
        try:
            response = (self.features.headset_bt_conn_info_feature.
                ...
                zone_vibe_wireless_api.dongle_bt_address))
            self.features.headset_bt_conn_info_feature.verify_get_device_connected_name(
                response, zone_vibe_wireless_api.receiver_name)
        except Exception as ex:
            Report.logException(str(ex))
    ...
    def test_1532_VC_80481_get_audio_active_device(self) -> None:
        try:
            ...
            self.features.headset_bt_conn_info_feature.verify_get_audio_active_device(
                response, zone_vibe_wireless_api.dongle_bt_address, "00")
        except Exception as ex:
            Report.logException(str(ex))
    ...
    def test_1533_VC_80482_get_dongle_fw_version(self) -> None:
        try:
            ...
            self.features.headset_bt_conn_info_feature.verify_get_dongle_fw_version(
                response, zone_vibe_wireless_api.dongle_firmware_version)
        except Exception as ex:
            Report.logException(str(ex))
        ...
        try:
            ...
            for status in statuses:
                self.features.headset_bt_conn_info_feature.set_A2DP_mute_status(
                    zone_vibe_wireless_api.dongle_bt_address, status)
                response = (self.features.headset_bt_conn_info_feature.
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(
        ZoneVibeWirelessApiDongleTests)
