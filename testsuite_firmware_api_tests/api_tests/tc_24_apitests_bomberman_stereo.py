...
from testsuite_firmware_api_tests.api_tests.api_parameters import bomberman_stereo_api
...
from testsuite_firmware_api_tests.api_tests.headsets_earbuds_firmware_api.centurion_features.features import \
    Features
from testsuite_firmware_api_tests.api_tests.headsets_earbuds_firmware_api.device_config_bomberman_stereo import *


class BombermanStereoAPITests(UIBase):
    """
    ...
    @classmethod
    def setUpClass(cls):
        super(BombermanStereoAPITests, cls).setUpClass()
        cls.centurion = CenturionCommands(device_name=bomberman_stereo_api.name,
                                          conn_type=ConnectionType.dongle)
        cls.features = Features(cls.centurion)
        global_variables.firmware_api_device_name = "BOMBERMAN_STEREO"
        global_variables.firmware_api_device_conn = ConnectionType.dongle

    def test_2401_VC_138717_get_protocol_version(self):
        try:
    ...
    def test_2402_VC_138716_get_features(self):
        try:
            response = self.features.root_feature.get_features(FEATURES_BOMBERMAN_STEREO)
            self.features.root_feature.verify_get_features_responses(response, FEATURES_BOMBERMAN_STEREO)
        except Exception as e:
            Report.logException(str(e))

    def test_2403_VC_138715_get_not_supported_feature(self):
        try:
    ...
    def test_2404_VC_138714_get_not_existing_feature(self):
        try:
    ...
    def test_2405_VC_138713_get_feature_count(self):
        try:
            response = self.features.feature_set_feature.get_feature_count()
            self.features.feature_set_feature.verify_feature_count(response, len(FEATURES_BOMBERMAN_STEREO))
        except Exception as e:
            Report.logException(str(e))

    def test_2406_VC_138712_get_feature_id(self):
        try:
            response = self.features.feature_set_feature.get_feature_id()
            self.features.feature_set_feature.verify_get_feature_id(response, FEATURES_BOMBERMAN_STEREO)
        except Exception as e:
            Report.logException(str(e))

    def test_2407_VC_138711_get_serial_number(self):
        try:
            response = self.features.device_info_feature.get_serial_number()
            self.features.device_info_feature.verify_serial_number(response, bomberman_stereo_api.serial_number)
        except Exception as e:
            Report.logException(str(e))

    def test_2408_VC_138710_get_firmware_version(self):
        try:
            response = self.features.device_info_feature.get_firmware_version()
            self.features.device_info_feature.verify_firmware_version(response, bomberman_stereo_api.firmware_version)
        except Exception as e:
            Report.logException(str(e))

    def test_2409_VC_138709_get_hardware_info(self):
        try:
            hw_revision = '00'

            response = self.features.device_info_feature.get_hardware_info()
            self.features.device_info_feature.verify_hardware_info(response, bomberman_stereo_api.model_id, hw_revision)
        except Exception as e:
            Report.logException(str(e))

    def test_2410_VC_138708_set_get_mic_mute_state(self):
        try:
            statuses = [0, 1]
    ...
    def test_2411_VC_138707_get_sidetone_level(self):
        try:
            response = self.features.headset_audio_feature.get_sidetone_level()
        except Exception as e:
            Report.logException(str(e))

    def test_2412_VC_138718_set_not_supported_value_of_sidetone_level(self):
        try:
    ...
    def test_2412_VC_138706_set_get_voice_notification_status(self):
        try:
            statuses = [0, 1]
            for status in statuses:
    ...
    def test_2414_VC_138704_factory_reset_device(self):
        try:
            self.features.headset_misc_feature.factory_reset_device()
        except Exception as e:
            Report.logException(str(e))

    def test_2415_VC_138703_set_get_anti_startle_state(self) -> None:
        try:
            statuses = [0, 1]
            for status in statuses:
    ...
    def test_2416_VC_138702_set_not_supported_anti_startle_state(self) -> None:
        try:
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(BombermanStereoAPITests)
