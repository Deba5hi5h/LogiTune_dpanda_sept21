...
class RaidenAPIC930eDevice(RaidenBaseAPI):
    """
    ...
    def setUpClass(cls):
        try:
            super(RaidenAPIC930eDevice, cls).setUpClass()
    ...
    def tearDownClass(cls):
        super(RaidenAPIC930eDevice, cls).tearDownClass()

    def setUp(self):
        super(RaidenAPIC930eDevice, self).setUp() #Extent Report
    ...
    def tearDown(self):
        super(RaidenAPIC930eDevice, self).tearDown() #Extent Report

    def test_001_VC_79964_Get_Device_Info_C930e(self):
        """Get Device Information in Personal Devices: C930e.
            Setup:
                  1. Sign in to Sync Portal using valid owner credentials.
                  2. Make sure that C930e is connected to the organization.

            Test:
                 1. Query the API: Get Call to
                 GET ~/org/{org-id}/host/Host-{host-id}/info
                 2. Search for accessory C930e and get the device information.

        """
        try:
            self.banner('Get Device Info: C930e')
            ...
            for accessory, object in response['accessories'].items():
                if object['name'] == 'C930e':
                    output = object
                    break

            json_formatted_response = json.dumps(output, indent=2)
            Report.logInfo('Response for GET info of C930e device')
            Report.logResponse(format(json_formatted_response))
            video_fw_version = output['fw']
            serial = output['serial']
            Report.logInfo('Serial - {}'.format(serial))
            Report.logInfo('Video Firmware - {}'.format(video_fw_version))
...
if __name__ == "__main__":
    test_suite = unittest.TestLoader().loadTestsFromTestCase(RaidenAPIC930eDevice)
