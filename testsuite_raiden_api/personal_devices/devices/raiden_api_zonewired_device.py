...
class RaidenAPIZoneWiredDevice(RaidenBaseAPI):
    """
    ...
    def setUpClass(cls):
        try:
            super(RaidenAPIZoneWiredDevice, cls).setUpClass()
    ...
    def tearDownClass(cls):
        super(RaidenAPIZoneWiredDevice, cls).tearDownClass()

    def setUp(self):
        super(RaidenAPIZoneWiredDevice, self).setUp() #Extent Report
    ...
    def tearDown(self):
        super(RaidenAPIZoneWiredDevice, self).tearDown() #Extent Report

    def test_001_VC_79966_Get_Device_Info_ZoneWired(self):
        """Get Device Information in Personal Devices: Zone Wired.
            Setup:
                  1. Sign in to Sync Portal using valid owner credentials.
                  2. Make sure that Zone Wired is connected to the organization.

            Test:
                 1. Query the API: Get Call to
                 GET ~/org/{org-id}/host/Host-{host-id}/info
                 2. Search for accessory Zone Wired and get the device information.

        """
        try:
            self.banner('Get Device Info: Zone Wired')
            ...
            for accessory, object in response['accessories'].items():
                if object['name'] == 'Zone Wired':
                    output = object
                    break
            json_formatted_response = json.dumps(output, indent=2)
            Report.logInfo('Response for GET info of Zone Wired device')
...
if __name__ == "__main__":
    test_suite = unittest.TestLoader().loadTestsFromTestCase(RaidenAPIZoneWiredDevice)
